...
function formatUserDisplayName(user: MostActiveUserData): string {
...
export function MostActiveUsersChart() {
  const { data, isLoading, error } = useQuery({
    queryKey: ["most-active-users-today"],
    queryFn: () => getMostActiveUsersToday(10, "UTC"),
  });
        ...
        >
          ...
          <div className="flex-shrink-0">
            <span className="inline-flex items-center rounded-full bg-primary/10 px-2.5 py-0.5 text-sm font-medium text-primary">
              {user.activity_count.toLocaleString()} activities
            </span>
          </div>
        </div>
      ))}
